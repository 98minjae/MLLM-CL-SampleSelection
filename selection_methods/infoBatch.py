...
def infobatch_load_state_dict(model, global_state_dict, local_state_dict_list, client_id, training_args, extra_state_dict_dict):
...
def infobatch_create_trainer(model, tokenizer, training_args, data_module, extra_state_dict_dict, curr_ema=None, curr_ema_var=0, datalist_prob=None, ema_data_module=None, ema_pool=None, return_task_vectors=False):
    trainer = LLaVATrainerInfoBatch(model=model,
...
def infobatch_aggregate_state_dict(global_state_dict, local_state_dict_list, selected_ids, num_selection, training_args, **kwargs):
    for key in global_state_dict.keys():
        global_state_dict[key] = sum([local_state_dict_list[client][key] / num_selection for client in selected_ids])

class LLaVATrainerInfoBatch(LLaVATrainerFEDAVG):
    def __init__(self, ema_ratio,mode,client_id, curr_round, task_vector=None, fisher_old=None, fisher_freq=5, return_embedding=None, return_task_vectors=False, selection_method=None, kmeans=None, curr_ema=None, curr_ema_var=0, min_z_score_threshold=0, max_z_score_threshold=0, start_ema_update_step=0, warmup_sample_ratio=0, datalist_prob=None, ema_update_mode=None, softmax_update_temp=0,ema_data_module=None, ema_average=False, ema_pool=None, ema_pool_size=0, eval_period=8000, **kwargs):
        super(LLaVATrainerInfoBatch, self).__init__(client_id=client_id, curr_round=curr_round, task_vector=task_vector, fisher_old=fisher_old, fisher_freq=fisher_freq, return_embedding=return_embedding, return_task_vectors=return_task_vectors, selection_method=selection_method, kmeans=kmeans, **kwargs)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if not self.return_task_vectors:
                    ...
                    with torch.no_grad():
                        with torch.inference_mode():
                            ...
                            # all_reps.append(reps.to(dtype=torch.float16))
                            info = output.loss.clone().detach().cpu()
                            ...
                            # info = torch.mean(torch.sum(entropy, dim=-1), dim=-1).detach().cpu().squeeze()
                    ...
                    if self.ema_average:
                        if len(self.ema_pool) >= self.ema_pool_size:
                            self.ema_pool.pop(0)
                        self.ema_pool.append(info.item())
                    
                    batch_inputs.append(copy.deepcopy(inputs))
                ...
                if (
                    ...
                    if self.curr_ema == None:
                        well_learned_mask = (torch.tensor(batch_info) < torch.tensor(batch_info).sum()/len(torch.tensor(batch_info))).numpy()
                    else:
                        well_learned_mask = (torch.tensor(batch_info) < torch.tensor(self.curr_ema)).numpy()
                    ...
                    if len(remained_indices)==0:
                        selected_indices = np.random.choice(well_learned_indices, self.sample_size_perbatch, replace=False)
                        upscale_tf = [True]*len(selected_indices)

                    elif len(remained_indices)<self.sample_size_perbatch:
                        selected_indices = copy.deepcopy(remained_indices)
                        upscale_tf = [False]*len(selected_indices)
                        selected_indices.extend(np.random.choice(well_learned_indices, self.sample_size_perbatch-len(remained_indices), replace=False))
                        upscale_tf.extend([True]*(self.sample_size_perbatch-len(remained_indices)))
                    else:
                        selected_indices = np.random.choice(remained_indices, self.sample_size_perbatch, replace=False)
                        upscale_tf = [False]*len(selected_indices)
                    
                    
                    selected_samples = [batch_inputs[ind] for ind in selected_indices]
                    self.selected_info.extend([batch_info[ind] for ind in selected_indices])
                    selected_stream_indices = [batch_inds[ind] for ind in selected_indices]
                    self.selected_info_ind.extend(selected_stream_indices)
                    ...
                    for i, sample in enumerate(selected_samples):
                        ...
                        with self.accelerator.accumulate(model):
                            tr_loss_step = self.training_step(model, sample, upscale=upscale_tf[i])
    ...
    def training_step(
        self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], num_items_in_batch=None, return_outputs=False, no_grad_cal=False, num_sample=1, upscale=False
    ) -> torch.Tensor:
        """
        ...
        upscale =True
        if upscale:
            loss /= (1-self.warmup_sample_ratio)
        ...
        if not no_grad_cal:
            if self.use_apex:
                ...
                # loss /= torch.tensor(num_sample).cuda()
