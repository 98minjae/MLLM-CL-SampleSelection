...
def prepare_calibration_input(model, batch):
    use_cache = model.base_model.model.config.use_cache
...
def fedavg_create_trainer(model, tokenizer, training_args, data_module, extra_state_dict_dict, return_task_vectors=False):
    trainer = LLaVATrainerFEDAVG(model=model,
        ...
        select_ratio=training_args.warmup_sample_ratio,
...
class LLaVATrainerFEDAVG(LLaVATrainer):
    def __init__(self, client_id, curr_round, task_vector=None, fisher_old=None, fisher_freq=5, return_embedding=None, return_task_vectors=False, selection_method=None, kmeans=None, select_ratio=None,eval_period=8000,**kwargs):
        ...
        self.return_task_vectors = return_task_vectors
    ...
    def _inner_training_loop(
        ...
        self.sample_size_perbatch = int(total_train_batch_size*self.select_ratio)
        len_dataloader = None
        ...
        if self.is_deepspeed_enabled:
            tr_loss_step, output = deepspeed_init(self, num_training_steps=max_steps)
        ...
        if resume_from_checkpoint is not None and os.path.isfile(
            ...
            if not args.ignore_data_skip:
                ...
                steps_trained_in_current_epoch *= args.gradient_accumulation_step*self.bs
            else:
                steps_trained_in_current_epoch = 0
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    all_indices = list(range(len(batch_inputs)))
                    selected_indices = random.sample(all_indices, k=self.sample_size_perbatch)
                    selected_inputs = [batch_inputs[i] for i in selected_indices]
    ...
    def training_step(
        self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], num_items_in_batch=None, return_outputs=False
    ) -> torch.Tensor:
        """
    ...
    def generate_llava_blocks(self):
        ### Vision Towers ###
    ...
    def model_trainable_block(self):
        trainable_block = [['base_model.model.model.mm_projector.0', 'base_model.model.model.mm_projector.2'],
