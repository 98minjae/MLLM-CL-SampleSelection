...
def infobudget_load_state_dict(model, global_state_dict, local_state_dict_list, client_id, training_args, extra_state_dict_dict):
...
def infobudget_prob_create_trainer(model, tokenizer, training_args, data_module, extra_state_dict_dict, curr_ema=None, curr_ema_var=0, datalist_prob=None, ema_data_module=None, ema_pool=None, return_task_vectors=False):
    trainer = LLaVATrainerInfoBudget(model=model,
...
def infobudget_aggregate_state_dict(global_state_dict, local_state_dict_list, selected_ids, num_selection, training_args, **kwargs):
    for key in global_state_dict.keys():
        global_state_dict[key] = sum([local_state_dict_list[client][key] / num_selection for client in selected_ids])

class LLaVATrainerInfoBudget(LLaVATrainerFEDAVG):
    def __init__(self, ema_ratio,mode,client_id, curr_round, task_vector=None, fisher_old=None, fisher_freq=5, return_embedding=None, return_task_vectors=False, selection_method=None, kmeans=None, curr_ema=None, curr_ema_var=0, min_z_score_threshold=0, max_z_score_threshold=0, start_ema_update_step=0, warmup_sample_ratio=0, datalist_prob=None, ema_update_mode=None, softmax_update_temp=0,ema_data_module=None, ema_average=False, ema_pool=None, ema_pool_size=0, eval_period=8000, **kwargs):
        super(LLaVATrainerInfoBudget, self).__init__(client_id=client_id, curr_round=curr_round, task_vector=task_vector, fisher_old=fisher_old, fisher_freq=fisher_freq, return_embedding=return_embedding, return_task_vectors=return_task_vectors, selection_method=selection_method, kmeans=kmeans, **kwargs)
    ...
    def _inner_training_loop(
        ...
        print("self.sample_size", self.sample_size_perbatch)
        len_dataloader = None
        ...
        # print("selected_num_per_batch", selected_num_per_batch)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                    ...
                    if self.selection_method == "fisher":
                        ...
                        for n, p in self.model.base_model.model.model.layers[-1].named_parameters():
                            if p.requires_grad == True:
                                ...
                                grad = torch.nn.functional.normalize(grad, p=2, dim=1)
                                if info==None:
                                    info = (grad**2).sum().cpu()
                                else:
                                    info += (grad**2).sum().cpu()
                ...
                if (
                    ...
                    if self.curr_ema == None or self.curr_ema_var==0:
                        batch_info_z = copy.deepcopy(batch_info_tensor)
                    else:
                        batch_info_z = (batch_info_tensor - torch.tensor(self.curr_ema)) / torch.tensor(math.sqrt(self.curr_ema_var))
                    sigmoid_scores = torch.sigmoid(batch_info_z - self.min_z_score_threshold)
                    threshold = torch.rand(1).item()
                    selected_mask = sigmoid_scores > threshold
                    selected_indices = torch.nonzero(selected_mask, as_tuple=False).view(-1).tolist()
                    ...
                    # batch_info_prob = batch_info_z / torch.tensor(self.softmax_update_temp)
                    # softmax_probs = torch.nn.Softmax(dim=0)(batch_info_prob).tolist()
                    # softmax_probs = [prob/sum(softmax_probs) for prob in softmax_probs]
                    # print("len(batch_inputs)", batch_inputs)
                    ...
                    # for n, p in model.base_model.model.model.layers[-1].self_attn.q_proj.lora_A.default.named_parameters():
                    #     print("pcheck2", p)
