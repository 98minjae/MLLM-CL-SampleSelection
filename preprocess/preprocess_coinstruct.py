...
def split_q_instruct(datalist, output_folder, train_samples, test_samples):
    ...
    json_data_list_train = json_data_list_1[:int(train_samples)] + json_data_list_2[:int(train_samples)]
    json_data_list_test = json_data_list_1[-int(test_samples):] + json_data_list_2[-int(test_samples):]
    ...
    total_train.extend(json_data_list_train)
    total_train.extend(json_data_list_2_train)
    # total_train.extend(json_data_list_3_train)
    
    total_train.extend(json_data_list_test)
    total_train.extend(json_data_list_2_test)
    # total_train.extend(json_data_list_3_test)
    
    total_size = len(total_train)
    total_test = total_train[int(0.8*total_size):]
    total_train = total_train[:int(0.8*total_size)]
    
    # Save the JSON data list to a file
    json_output_path = os.path.join(train_folder, f'dataset-99.json')
    print(f"Total samples: {len(total_train)}")
    with open(json_output_path, 'w') as json_file:
        json.dump(total_train, json_file, indent=4)
        
    json_output_path = os.path.join(test_folder, f'dataset-99.json')
    print(f"Total samples: {len(total_test)}")
    total_test = random.sample(total_test, 2000)
    with open(json_output_path, 'w') as json_file:
        json.dump(total_test, json_file, indent=4)
...
split_q_instruct(original_q_instruct, 'dataset/Co-Instruct-DB', int(200277*0.8), int(200277*0.2))
